...
def normalize_signal(
    ...
    # Confidence = average agent support for the final direction, so a signal
    # contradicted by its own agents can no longer show 95%. Each vote
    # contributes its score when it agrees with the signal, (100 - score)
    # when it opposes, and 50 when either side is neutral.
    support = []
    for v in agent_votes.values():
        if v["vote"] == default_vote:
            support.append(v["score"])
        elif v["vote"] == "neutral" or default_vote == "neutral":
            support.append(50)
        else:
            support.append(100 - v["score"])
    confidence = min(0.95, max(0.1, sum(support) / (100 * len(support))))
